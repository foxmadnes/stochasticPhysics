...
function InitializeDemo() {
  ...
  ripl.assume('initial-pos-x',parse('(mem (normal 0.0 1.0))'));
  ripl.assume('initial-vel-x',parse('(mem (normal 0.0 1.0))'));
  ripl.assume('force-x',parse('(mem (normal 0.0 1.0))'));
  predicted_coords[0] = ripl.assume('pos-x',parse('(mem (lambda (time) (if (= time c[0]) (initial-pos-x) (+ pos-x (dec time))(initial-vel-x)(force-x)(normal 0.0 0.01))'));
  
  ripl.assume('initial-pos-y',parse('(mem (normal 0.0 1.0))'));
  ripl.assume('initial-vel-y',parse('(mem (normal 0.0 1.0))'));
  ripl.assume('force-y',parse('(mem (+ 9.8 (normal 0.0 1.0)))'));
  predicted_coords[1] = ripl.assume('pos-y',parse('(mem (lambda (time) (if (= time c[0]) (initial-pos-y) (+ pos-y (dec time))(initial-vel-y)(force-y)(normal 0.0 0.01))'));
  ripl.start_cont_infer(1); // Start the continuous ("infinite") inference.

  // Prepare the canvas in your browser.
  var canvas = d3.select('#graphics_div').append("canvas").attr("width",420).attr("height",420);
...
function drawPath(points) {
  ...
  canvas.append("circle").attr("cx",points[0][0]).attr("cy",points[0][1]);
  for (point in points) {
